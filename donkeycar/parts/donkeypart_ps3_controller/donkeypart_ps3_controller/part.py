...
#        print('Opening {}...'.format(self.dev_fn))
...
#        print('Number of buttons {}'.format(self.num_buttons))
 ...
 #       print('axis name {}'.format(self.axis_map))
...
class PS3Joystick(Joystick):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.axis_names = {
            0x00: 'left_stick_horz', # wangbin
            0x01: 'left_stick_vert', # wangbin
        }

        self.button_names = {

            0x130: "char_Y",  # 0 move forward
            0x132: "char_A",  # 2 move backward
            0x133: "char_X",  # 3 emergency stop
            0x136: 'L2',      # 6 310 # turn left
            0x137: 'R2',      # 7 311 # turn right
...
class JoystickController(object):
    ...
    def increase_max_throttle(self):
        ...
        '''
        self.throttle_scale = round(min(1.0, self.throttle_scale + 0.10), 2)
    ...
    def decrease_max_throttle(self):
        ...
        '''
        self.throttle_scale = round(max(0.0, self.throttle_scale - 0.10), 2)
        if self.constant_throttle:
            self.throttle = self.throttle_scale
            self.on_throttle_changes()
...
#        self.chaos_monkey_steering = -0.2
        self.chaos_monkey_steering = -1.0
...
class PS3JoystickController(JoystickController):
    ...
    def init_trigger_maps(self):
        ...
        self.button_down_trigger_map = {
            'R2': self.chaos_monkey_on_right,
            'L2': self.chaos_monkey_on_left,
            'char_X': self.emergency_stop,
            'char_Y': self.increase_max_throttle,
            'char_A': self.decrease_max_throttle,
        }

        self.button_up_trigger_map = {
            "R2": self.chaos_monkey_off,
            "L2": self.chaos_monkey_off,
        }
